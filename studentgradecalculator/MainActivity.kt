...
import com.example.studentgradecalculator.ui.theme.StudentGradeCalculatorKotlinVersionTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val students = listOf(
            Student(name = "PRECIOUS",    score = 85.5),
            Student(name = "RITA",     score = null),
            Student(name = "PAOLA", score = 90.0),
            Student(name = "GLENYS",   score = 93.0),
            Student(name = "BRIGHTON",    score = 70.0),
            Student(name = "TREASURE",    score = 99.5),
            Student(name = "LAURA",    score = 89.5),
            Student(name = "JENNY",    score = 52.0)
        )
...
fun StudentReportScreen(
    ...
    Column(
        ...
        Text(
            ...
            color = Color(0xFF00FFFF), // Neon blue for title
        ...
        gradeSummaries.forEach { summary ->
            Text(
                text = summary,
                fontSize = 14.sp,
                color = Color(0xFF00FFFF), // Neon blue for summary text
        ...
        if (atRiskStudents.isEmpty()) {
            Text(text = "No at-risk students.", fontSize = 14.sp, color = Color(0xFF00FFFF))
            Spacer(modifier = Modifier.height(12.dp))
        } else {
        ...
        if (honourRoll.isEmpty()) {
            Text(text = "No students achieved an A.", fontSize = 14.sp, color = Color(0xFF00FFFF))
            Spacer(modifier = Modifier.height(12.dp))
        } else {
...
fun StudentCard(student: Student) {
    ...
    // Different shades of neon blue for grades
    val gradeColor = when (grade) {
        "A"  -> Color(0xFF4DFFFF) // Bright neon blue
        "B"  -> Color(0xFF3DF2FF) // Slightly deeper neon blue
        "C"  -> Color(0xFF2DE5FF) // Medium neon blue
        "D"  -> Color(0xFF1DD8FF) // Darker neon blue
        else -> Color(0xFF0DCBFF) // Deepest neon blue for F
    }
    ...
    Card(
        ...
        colors = CardDefaults.cardColors(
            containerColor = Color(0xFF1A1A1A) // Dark background to make neon pop
        )
    ) {
        Column(modifier = Modifier.padding(16.dp)) {
            Text(
                ...
                fontSize = 18.sp,
                fontWeight = FontWeight.Bold,
                color = Color(0xFF00FFFF) // Neon blue for name
            )
            Spacer(modifier = Modifier.height(6.dp))
            Text(
                text = "Score : $scoreDisplay",
                fontSize = 15.sp,
                color = Color(0xFF00FFFF) // Neon blue for score
            )
            ...
            Text(
                ...
                fontSize = 12.sp,
                color = Color(0xFF88FFFF) // Lighter neon blue for summary
            )
        }
    }
}
